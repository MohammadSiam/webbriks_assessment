...
import { createBoardSchema, type CreateBoardInput } from "@webbriks/shared-types";
import { apiRequest, ApiError } from "@/lib/api-client";
import { Modal } from "@/components/ui/modal";

export function BoardCreateModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  const {
    register,
    handleSubmit,
    setError,
  ...
  const onSubmit = async (values: CreateBoardInput) => {
    try {
      await apiRequest("/boards", { method: "POST", data: values });
      onCreated();
      onClose();
    } catch (err) {
  ...
  return (
    <Modal title="New board" onClose={onClose}>
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-3">
        ...
        <div className="flex flex-col gap-1">
          <label htmlFor="new-board-description" className="text-sm font-medium">
            Description
          </label>
          <textarea
